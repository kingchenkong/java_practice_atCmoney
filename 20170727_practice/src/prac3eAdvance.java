...
public class prac3eAdvance {
	...
	public static void main(String[] args) {
		...
		//for test
		//		int[][] array2axisCoordinate = new int[3] [];
		//		array2axisCoordinate[0] = new int[] {1, 2, 3, 4, 5};
		//		array2axisCoordinate[1] = new int[] { 20, 0, 0, 100, 50};
		//		array2axisCoordinate[2] = new int[] { 25, 45, 0, 25, 60};
		//		int maxScanX = 1000, maxScanY = 1000;
		//		int minScanX = 0, minScanY = 0;
		...
		while(cycleOption) {
			...
			case 2:		//output
				...
				for(int i = 0; i < alreadyNewCount; i++) {
					//建立需要的array數量
					arrayDistance[i] = new double[alreadyNewCount - i - 1]; 

					for(int j = i + 1; j < alreadyNewCount; j++) {
						...
						if( (distance < arrayMinDistance[j]) && (i != j) ){
							arrayMinDistance[j] = distance;
							arrayMostClosePointIndex[j] = i;
						}
				...
				for(int j = 0; j < alreadyNewCount; j++) {
					System.out.printf("( %4d, %4d ) ->\t", array2axisCoordinate[1][j], array2axisCoordinate[2][j]);
					//					if(array2axisCoordinate[1][j] < 10) 
					//						System.out.print(" ");

					System.out.printf("最近的點: ( %4d, %4d )\t", array2axisCoordinate[1][ arrayMostClosePointIndex[j] ], array2axisCoordinate[2][ arrayMostClosePointIndex[j ]]);
					System.out.printf(" 最短距離: %10.2f\n", arrayMinDistance[j]);
				}
				System.out.println("**************************************");
